...
public class TiledLevelGenerator : MonoBehaviour
{
    ...
    void Generate()
    {
        tilesetData = new TilesetData[FileInfo.TiledFile.Map.Tilesets.Length];
        for (int i = 0; i < FileInfo.TiledFile.Map.Tilesets.Length; i++)
        {
            var tileset = FileInfo.TiledFile.Map.Tilesets[i];
        ...
        for (int i = 0; i < FileInfo.TiledFile.Map.Layers.Length; i++)
        {
            ...
            int x = 0;
            int y = 0;
            float zOrder = (FileInfo.TiledFile.Map.Layers.Length - 1 - i) * ZOrderDepth;
            ...
            for (int j = 0; j < layer.data.tiles.Length; j++)
            {
                ...
                if (x >= FileInfo.TiledFile.Map.width-1)
                {
                    x = 0;
                    y--;
                }
    ...
    {
        public TiledFileInfo.TiledTileset Tileset;
        public Sprite[] Sprites;

        internal TiledFileInfo.TiledTilesetTileConfig GetConfig(long gid)
        {
